  ...
  async function renderWidgets() {
    ...
    widgetsList.querySelectorAll("[data-copy]").forEach((button) => {
      button.addEventListener("click", async () => {
        ...
        if (!widget) return;
        await navigator.clipboard.writeText(widgetCode(widget, host));
    ...
    async function payFor(widgetId, tariff, button) {
      ...
      try {
        ...
        if (!checkout.checkoutUrl) throw new Error("Ссылка оплаты не получена");
    ...
    widgetsList.querySelectorAll("[data-pay-monthly]").forEach((button) => {
      button.addEventListener("click", () => {
        payFor(button.getAttribute("data-pay-monthly"), "monthly", button);
      });
    });
    widgetsList.querySelectorAll("[data-pay-yearly]").forEach((button) => {
      button.addEventListener("click", () => {
        payFor(button.getAttribute("data-pay-yearly"), "yearly", button);
      });
    });
  }
